...
import dtd.DTDGenerator.ChildDetails;
import dtd.DTDGenerator.ElementDetails;
 ...
 * all it needs is: JAXP 1.1 SAX2 A JAXP 1.1 conform XML parser Java 1.2
 * 
 * @author M.H.Kay
...
public class DTDGenerator extends org.xml.sax.helpers.DefaultHandler {
	...
	// minimum number of attribute values that must appear
	...
	TreeMap<String, ElementDetails> elementList; // alphabetical list of element
													// types appearing in the
													// document;
	 ...
	 * test, it treats all non-ASCII characters as being valid in names.
	 */
	 ...
	 * test, it treats all non-ASCII characters as being valid in NMTOKENs.
	 */
		...
		try {
			...
			while (e.hasNext()) {
				...
				if (childKeys.size() == 0 && !ed.hasCharacterContent)
					fileWriter.print("<!ELEMENT " + elementname + " EMPTY >\n");

				// CHARACTER content
				if (childKeys.size() == 0 && ed.hasCharacterContent)
					fileWriter.print("<!ELEMENT " + elementname + " ( #PCDATA ) >\n");

				// ELEMENT content
				if (childKeys.size() > 0 && !ed.hasCharacterContent) {
					fileWriter.print("<!ELEMENT " + elementname + " ( ");
					...
					if (ed.sequenced) {
						...
						while (c1.hasNext()) {
							fileWriter.print((String) c1.next());
							if (c1.hasNext())
								System.out.print(" | ");
						}
						fileWriter.print(" )* >\n");
				...
				if (childKeys.size() > 0 && ed.hasCharacterContent) {
					...
					while (c2.hasNext()) {
						fileWriter.print(" | " + (String) c2.next());
					}
					fileWriter.print(" )* >\n");
				}
				;
				...
				while (a.hasNext()) {
					...
					// this may give the wrong answer, we should check whether
					...
					fileWriter.print("<!ATTLIST " + elementname + " " + attname + " ");
					...
					if (isid) {
						...
						fileWriter.print(tokentype + " #FIXED \"" + escape(val) + "\" >\n");
					} else if (isenum) {
						fileWriter.print("( ");
						Iterator v = ad.values.iterator();
						while (v.hasNext()) {
							fileWriter.print((String) v.next());
							if (!v.hasNext())
								break;
							fileWriter.print(" | ");
						}
					...
					if (!isfixed) {
						if (required)
							fileWriter.print(" #REQUIRED >\n");
						else
							fileWriter.print(" #IMPLIED >\n");
					}
				}
				;
				fileWriter.println("");
	...
	public void generateDtd(String filename) {

		run(filename + ".xml");
		printDTD(filename + ".dtd");
	}
